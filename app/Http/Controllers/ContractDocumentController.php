...
final class ContractDocumentController extends Controller
{
    ...
    public function store(Request $request, ContractDocumentReader $reader)
    {
        ...
        if (!in_array($extension, ['docx', 'xlsx'])) throw ValidationException::withMessages(['document' => 'DOCX veya XLSX yükleyin. Eski DOC/XLS belgelerini Office üzerinden yeni biçimde kaydedin.']);
        ...
        $text = $reader->read($bytes, $extension);
        try {
            $response = Http::withToken(config('services.openai.key'))->acceptJson()->connectTimeout(10)->timeout(100)
                ->post('https://api.openai.com/v1/responses', [
                    ...
                    'input' => json_encode(['selectedHotel' => $data['hotelName'], 'allowedRoomTypes' => $data['roomTypes'], 'document' => $text], JSON_UNESCAPED_UNICODE),
