export function copyAgencyContract(source, mode, amount, newId = () => crypto.randomUUID()) {
    ...
    const adults = row => {
        if (!/^\d+$/.test(String(row.pax)) || !Number.isSafeInteger(Number(row.pax))) throw Error('Kâr hesabı için her fiyat satırında geçerli yetişkin sayısı bulunmalıdır.');
        return Number(row.pax);
    };
    const mark = (value, count = 1, base = value) => {
        if (value === '' || value === null || value === undefined) return value;
        if (!/^\d+(?:\.\d+)?$/.test(String(value))) throw Error('Kaynakta geçersiz fiyat var.');
        const price = Number(value);
        if (mode === 'percent' && !/^\d+(?:\.\d+)?$/.test(String(base))) throw Error('Yüzde kârı için kontratın kişi başı ana fiyatı geçerli olmalıdır.');
        const addition = mode === 'percent' ? Number(base) * count * profit / 100 : profit * count;
        const result = Math.round((price + addition) * 100 + 1e-7) / 100;
    ...
    const contracts = source.contracts.map(original => {
        ...
        copy.prices = original.prices.map(row => ({ ...JSON.parse(JSON.stringify(row)), id: newId(), price: mark(row.price, adults(row), original.price), manualPrice: true, manualPriceEdited: false }));
        copy.conditions = copy.conditions.map(row => ({ ...row, id: newId() }));
        copy.rules = copy.rules.map(row => ({ ...row, id: newId() }));
        return copy;
    });
    return { id: newId(), name: source.name, currency, sourceSeasonId: source.id, markupMode: mode, markupAmount: profit, markupBasis: 'adult', contracts };
}
