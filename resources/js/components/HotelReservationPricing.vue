...
const props = defineProps<{ reservation: { checkIn:string; checkOut:string; roomCount:string }; contract?: HotelContract; agencyContract?: HotelContract; saved:boolean }>();
const rowId = ref('');
...
const agencyResult = computed(() => { const rows = matches(props.agencyContract); return reservationPricePreview(props.reservation, props.agencyContract, rows.length === 1 ? rows[0] : undefined); });
const profit = computed(() => result.value.total !== null && agencyResult.value.total !== null && result.value.currency === agencyResult.value.currency ? Math.round((agencyResult.value.total - result.value.total)*100)/100 : null);
...
<template>
    <section class="pricing-breakdown">
        ...
        <template v-else>
            ...
            <p>Net kâr = Acente fiyatı − Otel fiyatı. Transfer, ekstralar ve diğer giderler dahil değildir.</p>
            <p v-if="agencyResult.total !== null">Acente: {{ money(agencyResult.unit) }} × {{ agencyResult.nights }} gece × {{ agencyResult.rooms }} oda</p>
