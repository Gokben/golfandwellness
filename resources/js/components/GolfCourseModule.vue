    ...
    <section class="course-module" aria-label="Golf Sahaları">
        ...
        <form v-else class="course-form" @submit.prevent="saveCourse">
            ...
            <div v-if="activeCardTab === 'update'" class="course-form-body">
                ...
                <section class="course-detail-section"><h3>Zorunlu Tee Time'lar</h3><div class="detail-table tee-time-table"><div class="detail-head"><span>Tarih</span><span>Saat</span><span>Pax</span><span>Fiyat</span><span>Para Birimi</span><span>Özel Teklif</span><span>Yayınlama</span><span>Satış</span><span>Opsiyon Tarihi</span><span>Sil</span></div><div v-for="(item,index) in teeTimes" :key="index" class="detail-row"><DateInput v-model="item.date" /><input v-model="item.time" type="time"><input v-model.number="item.pax" type="number" min="1"><input v-model="item.price"><select v-model="item.currency"><option v-for="currency in currencyCodes" :key="currency">{{ currency }}</option></select><label class="check-cell"><input v-model="item.special" type="checkbox"></label><label class="check-cell"><input v-model="item.hidden" type="checkbox"></label><input :value="teeTimeSales(item, reservationStore.records.value)" type="number" disabled><DateInput v-model="item.optionDate" /><VoxActionButton action="delete" title="Sil" @click="teeTimes.splice(index,1)" /></div></div><button type="button" class="add-row" @click="addTeeTime">⊕ Ekle</button></section>
                <section class="course-detail-section"><h3>Golf Sahası Kapalı</h3><div class="detail-table closing-table"><div class="detail-head"><span>Kapalı Tarih #1</span><span>Kapalı Tarih #2</span><span>Buggy Tarih #1</span><span>Buggy Tarih #2</span><span>Açıklama</span><span>Sil</span></div><div v-for="(item,index) in closings" :key="index" class="detail-row"><DateInput :range-end="item.closedTo" v-model="item.closedFrom" /><DateInput :range-start="item.closedFrom" v-model="item.closedTo" /><DateInput :range-end="item.buggyTo" v-model="item.buggyFrom" /><DateInput :range-start="item.buggyFrom" v-model="item.buggyTo" /><input v-model="item.description"><VoxActionButton action="delete" title="Sil" @click="closings.splice(index,1)" /></div></div><button type="button" class="add-row" @click="addClosing">⊕ Ekle</button></section>
                <section class="course-detail-section"><h3>Golf Ekstraları</h3><div class="detail-table extras-table"><div class="detail-head"><span>İlk Tarih</span><span>Son Tarih</span><span>Açıklama</span><span>Alış Fiyatı</span><span>Satış Fiyatı</span><span>Para Birimi</span><span>P.T.</span><span>Zorunluluk</span><span>Sil</span></div><div v-for="(item,index) in extras" :key="index" class="detail-row"><DateInput :range-end="item.lastDate" v-model="item.firstDate" /><DateInput :range-start="item.firstDate" v-model="item.lastDate" /><select v-model="item.description"><option value="">Seçiniz</option><option v-for="name in extraNames" :key="name">{{ name }}</option></select><input v-model="item.buyPrice" type="number" min="0"><input v-model="item.sellPrice" type="number" min="0"><select v-model="item.currency"><option v-for="currency in currencyCodes" :key="currency">{{ currency }}</option></select><select v-model="item.type"><option>PP</option><option>PA</option></select><label class="check-cell"><input v-model="item.obligation" type="checkbox"></label><VoxActionButton action="delete" title="Sil" @click="extras.splice(index,1)" /></div></div><button type="button" class="add-row" @click="addExtra">⊕ Ekle</button></section>
            </div>
            ...
            <footer class="course-form-actions"><button type="submit" class="save" :disabled="blocked">{{ contractsBusy || busy || saving ? 'İşlem sürüyor…' : 'Kaydet' }}</button></footer>
        </form>
    </section>
</template>
