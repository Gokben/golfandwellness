...
<template><section class="exchange-currency"><header><h2>Döviz Kurları</h2><label>Tarih <input v-model="selectedDate" :disabled="loading" type="date" @change="loadRates"></label><small>{{ updatedAt }}</small><button type="button" :disabled="loading" @click="refreshRates">{{ loading ? 'Yenileniyor…' : '↻ TCMB’den Yenile' }}</button></header><p v-if="error" class="error">{{ error }}</p><div class="currency-table"><table><thead><tr><th>Ad</th><th>Döviz Alış</th><th>Döviz Satış</th><th>Efektif Alış</th><th>Efektif Satış</th><th>Tarih</th></tr></thead><tbody><tr v-for="rate in rates" :key="rate.name"><td><b>{{ rate.name }}</b></td><td>{{ rate.forexBuying }}</td><td>{{ rate.forexSelling }}</td><td>{{ rate.banknoteBuying }}</td><td>{{ rate.banknoteSelling }}</td><td>{{ displayDate(loadedDate) }}</td></tr></tbody></table></div></section></template>
