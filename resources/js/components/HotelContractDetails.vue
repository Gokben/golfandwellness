...
async function saveBookingDates(contracts: HotelContract[]) {
    ...
    const hotel = hotelStore.records.value.find(row => row.name === props.hotelName && contracts.every(contract => row.details?.contracts?.some(saved => saved.id === contract.id)));
    if (!hotel) { await voxAlert('Kaydedilmiş ana kontrat bulunamadı. Önce otel kaydını kaydedin.', 'error'); return; }
    savingDates.value = true;
    try {
        const next = hotelStore.records.value.map(row => row.id !== hotel.id ? row : {
            ...row, details: { ...row.details!, contracts: row.details!.contracts!.map(contract => ids.has(contract.id) ? { ...contract, bookingFirstDate: first, bookingLastDate: last } : contract) },
        });
        if (!await hotelStore.commit(next)) await voxAlert(hotelStore.storageError.value || 'Tarihler kaydedilemedi.', 'error');
    } finally { savingDates.value = false; }
}
...
<style scoped>
.agency-copy-action.vox-action-button { --action-color: #e45555; margin-left: 16px; vertical-align: middle; }
