...
async function save() {
    ...
    next.persons = form.value.persons.filter(hasPersonDetails);
  ...
  <section class="reservations-module">
    ...
    <template v-else>
      ...
      <form v-show="!props.combined || activeStep !== 3" class="reservation-form" @submit.prevent="save" @invalid.capture="revealInvalid">
        <div v-show="activeStep === 1" class="reservation-accordions"><button type="button" class="back-to-list" @click="screen = 'list'">Listeye dön</button><details open class="reservation-accordion"><summary>Adım #1 <em>(Oluşturan: {{ form.createdBy || (editingId ? 'Eski kayıtta bulunmuyor' : 'Kayıtta atanacak') }} / {{ form.createdAt ? new Date(form.createdAt).toLocaleDateString('tr-TR') : '—' }})</em></summary><div class="reservation-grid">
          ...
          <label>Otel Kontratı<RecordSelect v-model="form.hotelContract" :choices="hotelContractChoices" :disabled="!linked.hotels.ready.value" /><small v-if="linked.hotels.ready.value && form.hotel && !hotelContractChoices.length">Seçili otel, giriş tarihi ve oda için aktif kontrat bulunamadı.</small></label><label>Acente<RecordSelect v-model="form.agency" :choices="linked.agencyChoices.value" required /></label><label>Voucher<input v-model="form.voucher" disabled></label><label>Acente Kontratı<select v-model="form.agencyContract" disabled title="Bu kayıt için tanımlı kontrat bulunmuyor"><option value="">Seçiniz</option></select></label><label>Pansiyon<RecordSelect v-model="form.pension" :choices="boardChoices" /></label><label>Durum<select v-model="form.state"><option>REQUEST</option><option>OPTION</option><option>CONFIRM</option></select></label>
