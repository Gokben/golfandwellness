...
const props = defineProps<{ persons:ReservationPerson[]; roomChoices:Choice[]; transferChoices:Choice[]; roomsReady:boolean; transfersReady:boolean; checkIn:string; lockedRoomType?:string }>();
watchEffect(() => {
    for (const person of props.persons) birthChanged(person);
    if (props.lockedRoomType !== undefined) {
        for (const person of props.persons) person.roomType = props.lockedRoomType;
    }
});
function add() { props.persons.push({ ...newReservationPerson(), roomType: props.lockedRoomType ?? '' }); }
function birthChanged(person:ReservationPerson) {
    if (!person.birthDate || !props.checkIn) { person.age = ''; return; }
...
<template>
    <section class="reservation-persons">
        <div class="persons-scroll"><table><thead><tr><th>Unvan</th><th>Ad Soyad</th><th>Doğum Tarihi</th><th>Yaş</th><th>Oda Tipi</th><th>Transfer</th><th>İşlemler</th></tr></thead>
...
<td><input v-model="person.name" :required="hasPersonDetails(lockedRoomType !== undefined ? { ...person, roomType: '' } : person)" maxlength="200" :aria-label="`Ad Soyad ${index+1}`"></td>
                <td><DateInput v-model="person.birthDate" :max="checkIn || undefined" :aria-label="`Doğum Tarihi ${index+1}`" /></td>
                <td><input :value="person.age" type="text" readonly :aria-label="`Yaş ${index+1}`" title="Giriş tarihine göre otomatik hesaplanır"></td>
                <td><RecordSelect v-model="person.roomType" :choices="roomChoices" :disabled="lockedRoomType !== undefined || !roomsReady" :aria-label="`Oda Tipi ${index+1}`" /></td>
